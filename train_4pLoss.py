...
if args.model_path is None :
    from models.HopeV1_4pLoss import HopeV1_4pLoss 
    model = HopeV1_4pLoss().to(device)
    ...
    for epoch_idx in range(args.epoch_num):
        ...
        for file in train_files:
            ...
            for data in dataloader:
                ...
                sp,cs,lt,pg = model(intensity,angle) 
                error_sp = lossfn(sp,labels230)
                error_cs = lossfn(cs,labels7)
                error_lt = lossfn(lt,labels6)
                error_pg = lossfn(pg,labels32)
                error = error_sp + error_cs + error_lt + error_pg
                error.backward()
                optimizer.step()
                ema.update()
                ...
                err_pg += error_pg.item()
                
        logger.info('[training]total_num: '+str(total_num)+
                    ',error: '+str(total_err/batch_cnt)+
                    ',error_sp: '+str(err_sp/batch_cnt)+
                    ',error_cs: '+str(err_cs/batch_cnt)+
                    ',error_lt: '+str(err_lt/batch_cnt)+
                    ',error_pg: '+str(err_pg/batch_cnt))
...
def test(epoch_idx):
    ...
    pg_total_acc =  MulticlassAccuracy(32,average='micro').to(device) 
    total_num = 0 
    total_sp_err = 0.0 
    total_cs_err = 0.0 
    total_lt_err = 0.0 
    ...
    logger.info('[testing]total_number: '+str(total_num)+
                ...
                ',pg_error: '+str(total_pg_err/batch_cnt)+
                ',total_pg_acc: '+str(pg_acc_val)
                )
    writer.add_scalar("train/cs_err",total_cs_err/batch_cnt,epoch_idx)
    writer.add_scalar("train/cs_acc",cs_acc_val,epoch_idx)
    writer.add_scalar("train/lt_err",total_lt_err/batch_cnt,epoch_idx)
    writer.add_scalar("train/lt_acc",lt_acc_val,epoch_idx)
    writer.add_scalar("train/pg_err",total_pg_err/batch_cnt,epoch_idx)
    writer.add_scalar("train/pg_acc",pg_acc_val,epoch_idx)
    ema.restore()
