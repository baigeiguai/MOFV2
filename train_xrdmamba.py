    ...
    for epoch_idx in range(args.epoch_num):
        ...
        for file in train_files:
            ...
            for data in dataloader:
                optimizer.zero_grad()
                intensity,index,labels230,labels7,labels6 = data[0].type(torch.float).to(device),data[3].to(device).type(torch.long),data[2].to(device),data[4].to(device),data[5].to(device)
                sp = model(intensity,index) 
        ...
        logger.info('[training]total_num: '+str(total_num)+',error: '+str(total_err/batch_cnt)+',error_sp: '+str(err_sp/batch_cnt))
...
def test():
    ...
    with torch.no_grad():
        for file in train_files:
            ...
            for data in dataloader:
                intensity , index,labels230,labels7,labels6 = data[0].type(torch.float).to(device),data[3].type(torch.long).to(device),data[2].to(device),data[4].to(device),data[5].to(device)
                sp_raw_logits = model(intensity,index)
