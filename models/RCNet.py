...
class RCNet(nn.Module):
    ...
    def __init__(self, 
        ...
        self.first_block_relu = nn.LeakyReLU()
        out_channels = base_filters
        ...
        self.final_relu2 = nn.LeakyReLU(inplace=True)


    def forward(self, x,idx=None):

        out = x.view(x.shape[0],self.in_channels,-1)
        ...
        # out = self.to_embed(out)
        # return self.final_relu2(out)
        out = self.do(out)
...
if __name__ == '__main__':
    inp = torch.rand((16,1,5000))
    model = RCNet(in_channels=1,n_block=16,n_classes=230)
    out = model(inp)
    print(out.shape)
