...
class HopeV1_3Sup2Sp(torch.nn.Module):    
    def __init__(self):
        super(HopeV1_3Sup2Sp,self).__init__()
        self.conv_module = ResTcn(p_dropout=0.25)
        self.att = AttentionModule()
        self.conv_feature_len = 1024
        self.att_feature_len = 32 
        self.cls_sp_first = torch.nn.Sequential(   
            torch.nn.Linear(self.conv_feature_len+self.att_feature_len,256),
        )
        self.cls_sp_second = torch.nn.Sequential(   
            torch.nn.Linear(256+32+7+6,230),
        )
        ...
        self.cls_pg = torch.nn.Sequential(
            torch.nn.Linear(self.conv_feature_len+self.att_feature_len,32),
        )
    def forward(self,intensity,angle):
        ...
        cs = self.cls_cs(x)
        pg = self.cls_pg(x)
        x = torch.concat([self.cls_sp_first(x),cs,lt,pg],dim=-1)
        return self.cls_sp_second(x),cs,lt,pg
