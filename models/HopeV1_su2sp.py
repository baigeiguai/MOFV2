...
class HopeV1_Su2Sp(torch.nn.Module):    
    def __init__(self):
        super(HopeV1_Su2Sp,self).__init__()
        ...
        self.cls_sp = torch.nn.Sequential(   
            torch.nn.Linear(self.conv_feature_len+self.att_feature_len+13,230),
    ...
    def forward(self,intensity,angle):
        ...
        lt = self.cls_lt(x)
        cs = self.cls_cs(x)
        x = torch.concat([x,cs,lt],dim=-1)
        return self.cls_sp(x),cs,lt
